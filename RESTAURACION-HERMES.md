...
| **GitHub `holmart/hermesBackup`** | Skills, scripts, config, memories, cron jobs, SOUL.md | `git clone` |
| **AWS Secrets Manager** | `.env` con todas las API keys y tokens | `aws secretsmanager get-secret-value` |
| **Este documento** | Systemd service, crontab, estructura | Lo estas leyendo |
...
```bash
mkdir -p ~/.hermes/{bin,scripts,memories,skills,cron,leads,logs,sessions,hooks,plugins,gateway}
```
...
~/.hermes/scripts/sync_env_to_aws.sh
```
...
- **Archivos**: Diariamente a las 12:00 UTC -> `holmart/hermesBackup` (GitHub)
- **Secrets**: Diariamente -> AWS Secrets Manager (`hermes-agent/env`)
- **Version source**: Diariamente -> `hermes_version.txt` en el backup

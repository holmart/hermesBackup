...
# Try plain JSON with some fields
payloads = [
    {"texto": "control de plagas"},
    {"cod_camara": ""},
    {"cod_camara": "01"},  # Amazonas?
    {"texto": "", "cod_camara": ""},
    {},
]
...
for i, payload in enumerate(payloads):
    ...
    try:
        ...
        if registros:
            # Show first few nit and razon_social
            for j, reg in enumerate(registros[:3]):
                print(f"  {j+1}. NIT={reg.get('nit')}, RazonSocial={reg.get('razon_social')}, Camara={reg.get('cod_camara')}")

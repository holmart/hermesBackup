...
# Try empty dataBody
payload_empty = {"dataBody": ""}
# Try a simple search term maybe?
payload_search = {"dataBody": "U2FsdGVkX1+BaD2kYt15J6eECAw7hdvT7QwkpuspWaflX1NHZClewG2crBIIr5dK2ygPVar+QdDWWaY2YQee6cPWAJEQlGot4JVMlwkWDxo/tR2p8/+X7PzaXoOBRQ7U"}  # same as original

for name, payload in [("empty", payload_empty), ("original", payload_search)]:
    print(f"\n--- Testing {name} ---")
    ...
    json_start = stdout.find('{"registros":')
    if json_start == -1:
    ...
    try:
        ...
        if registros:
            # Show first record nit and razon_social
            first = registros[0]
            print(f"Primer registro: NIT={first.get('nit')}, RazonSocial={first.get('razon_social')}")
